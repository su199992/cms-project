      ...
      <Box>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 2 }}>
          <Box sx={{ display: "flex" }}>
            ...
            <Button onClick={handleInfoAdd} color="inherit" size="small">
              <AddIcon />
            </Button>
          </Box>
          <Box>
            <Button onClick={handleInfoClose} color="inherit" size="small">
              <CloseIcon />
            </Button>
          </Box>
        </Box>
            ...
            {[...rows, ...newRows].map((row) => (
              <ListItem key={row.id} alignItems="flex-start">
                ...
                {(editMode && editMode.id === row.id) || row.isNew ? (
                  <Box sx={{ display: "flex", width: "100%" }}>
                    <Box>
                      <TextField
                        ...
                        onChange={(e) =>
                          row.isNew ? handleInfoChange(row.id, "name", e.target.value) : setEditMode({ ...editMode, name: e.target.value })
                        }
                        placeholder="담당자 이름"
                        fullWidth
                        variant="standard"
                      ...
                      <TextField
                        ...
                        onChange={(e) =>
                          row.isNew ? handleInfoChange(row.id, "info", e.target.value) : setEditMode({ ...editMode, info: e.target.value })
                        }
                  ...
                  <>
                    <ListItemText primary={<span>{row.name}</span>} secondary={<span>{row.info}</span>} />
                    {!row.isNew && (
                      <Button onClick={() => handleInfoEdit(row.id, row.name, row.info)}>
                        <EditOutlinedIcon />
                      </Button>
                    )}
                    {!row.isNew && (
